...
const SHEET_NAME = 'bonos_totales';
...
function assertValidPayload(data) {
  const parsed = bonoTotalSchema.safeParse(data);
  if (!parsed.success) {
    const firstIssue = parsed.error.issues[0];
    throw new BadRequestError(firstIssue?.message || 'Datos invalidos para bono total.', {
      context: {
        sheet: SHEET_NAME,
...
function sortBonosForList(rows) {
...
function filterBonos(bonos, filters = {}) {
  return bonos.filter((bono) => matchesExactField(bono.caja, filters.caja));
}
...
async function upsert(data, caller) {
  ...
  if (existing) {
    ...
    await audit.log('upsert_overwrite', 'bono_total', getAuthLabel(caller), {
  ...
  await audit.log('create', 'bono_total', getAuthLabel(caller), record);
...
async function getPagedAndFiltered(filters = {}, limit, offset) {
  const bonos = await getAll();
  const filtered = filterBonos(bonos, filters);
  return paginateItems(sortBonosForList(filtered), limit, offset);
}
...
async function getLatest() {
  ...
  return sortBonosForList([...latest.values()]);
}
