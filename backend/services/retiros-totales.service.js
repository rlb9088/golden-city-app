...
const SHEET_NAME = 'retiros_totales';
...
function assertValidPayload(data) {
  const parsed = retiroTotalSchema.safeParse(data);
  if (!parsed.success) {
    const firstIssue = parsed.error.issues[0];
    throw new BadRequestError(firstIssue?.message || 'Datos invalidos para retiro total.', {
      context: {
        sheet: SHEET_NAME,
...
function sortRetirosForList(rows) {
...
function filterRetiros(retiros, filters = {}) {
  return retiros.filter((retiro) => matchesExactField(retiro.caja, filters.caja));
}
...
async function upsert(data, caller) {
  ...
  if (existing) {
    ...
    await audit.log('upsert_overwrite', 'retiro_total', getAuthLabel(caller), {
  ...
  await audit.log('create', 'retiro_total', getAuthLabel(caller), record);
...
async function getPagedAndFiltered(filters = {}, limit, offset) {
  const retiros = await getAll();
  const filtered = filterRetiros(retiros, filters);
  return paginateItems(sortRetirosForList(filtered), limit, offset);
}
...
async function getLatest() {
  ...
  return sortRetirosForList([...latest.values()]);
}
