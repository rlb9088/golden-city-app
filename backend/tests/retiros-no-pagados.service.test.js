...
function loadRetirosNoPagadosService({ getAll, getTable }) {
  ...
  const servicePath = require.resolve('../services/retiros-no-pagados.service');
  ...
  return {
    service: require('../services/retiros-no-pagados.service'),
    appendCalls,
    updateCalls,
    auditCalls,
  };
...
test('upsert crea un retiro no pagado nuevo', async () => {
  const { service, appendCalls, updateCalls, auditCalls } = loadRetirosNoPagadosService({
    getAll: async () => ([]),
  });
  ...
  assert.equal(auditCalls[0][1], 'retiro_no_pagado');
});
...
test('upsert rechaza una caja_id inexistente', async () => {
  const { service } = loadRetirosNoPagadosService({
    getAll: async () => ([]),
    getTable: async () => ([]),
  });
...
test('getPagedAndFiltered ordena por fecha descendente y caja ascendente', async () => {
  ...
  assert.deepStrictEqual(
    ...
    [
      ['RNP-2', '2026-05-04', 'Caja 2'],
      ['RNP-1', '2026-05-02', 'Caja 2'],
    ],
  );
...
test('getLatest devuelve el ultimo registro por caja', async () => {
  const { service } = loadRetirosNoPagadosService({
    getAll: async () => ([
      {
        id: 'RNP-1',
  ...
  assert.deepStrictEqual(
    result.map((row) => [row.id, row.caja_id, row.fecha]),
    [
      ['RNP-3', 'CJ-1', '2026-05-04'],
      ['RNP-2', 'CJ-2', '2026-05-04'],
    ],
  );
});
