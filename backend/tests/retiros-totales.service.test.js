...
function loadRetirosTotalesService({ getAll, getTable }) {
  ...
  const servicePath = require.resolve('../services/retiros-totales.service');
  ...
  return {
    service: require('../services/retiros-totales.service'),
    appendCalls,
    updateCalls,
    auditCalls,
  };
...
test('upsert crea un retiro total nuevo', async () => {
  const { service, appendCalls, updateCalls, auditCalls } = loadRetirosTotalesService({
    getAll: async () => ([]),
  });
  ...
  assert.equal(auditCalls[0][1], 'retiro_total');
});
...
test('upsert rechaza una caja_id inexistente', async () => {
  const { service } = loadRetirosTotalesService({
    getAll: async () => ([]),
    getTable: async () => ([]),
  });
...
test('getPagedAndFiltered ordena por fecha descendente y caja ascendente', async () => {
  ...
  assert.deepStrictEqual(
    ...
    [
      ['RTR-2', '2026-05-04', 'Caja 2'],
      ['RTR-1', '2026-05-02', 'Caja 2'],
    ],
  );
